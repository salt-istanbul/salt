    ...
    function realhero_ajax_subscribe() {
        ...
        if ( !empty( $fields ) ) :
            ...
            if( $count > 0 ) {
                $output = array(
                    'status'    => 'error',
                    'msg'       => __( 'You are already our subscriber.', $GLOBALS['text_domain'] )
                );
            } elseif( !defined( 'NEWSLETTER_VERSION' ) ) {
                $output = array(
                    'status'    => 'error',
                    'msg'       => __( 'Please install & activate newsletter plugin.', $GLOBALS['text_domain'] )
                );           
            } else {
                /**
                 * Generate token
                ...
                $output = array(
                    'status'    => 'success',
                    'msg'       => __( 'Thank you!', $GLOBALS['text_domain'] )
                );  
            }
            ...
            $output = array(
                'status'    => 'error',
                'msg'       => __( 'Error..', $GLOBALS['text_domain']  )
            );
        endif;
