...
function woo_get_cart_object(){
    ...
    foreach($items as $item => $values) {
            ...
            $image = get_the_post_thumbnail_url($id, "shop_thumbnail"); //$_product->get_image("shop_thumbnail");//$getProductDetail->get_image("shop_thumbnail"); // accepts 2 arguments ( size, attr )
...
function woo_get_product_variations_thumbnails($product_id, $size){//full, gallery_thumbnail, thumb
    $images = array();
    $variation_colors = array();
    $product = new WC_Product_Variable( $product_id );
    $variations = $product->get_available_variations();
    foreach ( $variations as $variation ) {
        if(array_key_exists("attribute_pa_renk", $variation["attributes"])){
            $variation_value = $variation["attributes"]["attribute_pa_renk"];
    ...
    if(empty($attr_value) || !isset($attr_value)){
        ...
        if($product_type == "variable" && !empty($attr)){
            ...
            if($variation_id){
                ...
                return woo_get_product_variations_thumbnails($product_id, $size);
            }
        }else{
...
function get_product_payments($product_id){
    $payments = array();
    $deposit_enabled = get_post_meta( $product_id, '_wc_deposits_enable_deposit')[0];
...
function get_cities($country_value="", $selected = ""){
    ...
    if($result){
      ...
      return $result;      
        ...
        return array(array(
            "name" => get_country_by_code($country_value),
            "slug" => $country_value
        ));
    }
}

function get_country_by_code($shortcode){
    $WC_Countries = new WC_Countries();
    $country_list = $WC_Countries->__get( "countries" );
    if(isset($country_list[$shortcode])){
       return $country_list[$shortcode];
    }else{
       return $shortcode;       
    }
}
...
function get_state_by_code($country, $state){
